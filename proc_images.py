...
path_to_images = '/Users/alex/baidu/mini-imagenet/images/'
...
for datatype in ['train', 'val', 'test']:
    datatype_dir = os.path.join('/Users/alex/baidu/mini_Imagenet','dataset',datatype)
    os.makedirs(datatype_dir)

    with open(os.path.join('/Users/alex/baidu/mini-imagenet/' ,datatype + '.csv'), 'r') as f:
        ...
        for i, row in enumerate(reader):
            ...
            image_name = row[0]
            if label != last_label:
                cur_dir = datatype_dir + '/' + label + '/'
                os.makedirs(cur_dir)
                last_label = label
            # os.system('cp images/' + image_name + ' ' + cur_dir)
            shutil.copy(os.path.join(path_to_images, image_name), os.path.join(cur_dir, image_name))
